...
class TestFoundation {
  ...
  public static void verifyKeystore(Object keystoreReference, String password, boolean production) {
    try {
      ...
      hackaton.communication.KeystoreManager.validateKeystoreParameter(keystoreReference);
    ...
    if (password != null) {
      try {
        ...
        hackaton.notification.AppleNotificationServer server = new hackaton.notification.AppleNotificationServerBasicImpl(keystoreReference, password, production);
        hackaton.communication.KeystoreManager.verifyKeystoreContent(server, keystoreReference);
