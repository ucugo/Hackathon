...
public class KeystoreManager {
  ...
  public static KeyStore loadKeystore(AppleServer server, Object keystore, boolean verifyKeystore) throws KeystoreException {
    ...
    synchronized (server) {
      try (InputStream keystoreStream = streamKeystore(keystore);) {
        ...
        throw new KeystoreException("Could not read from keystore stream.", ie);
      }
    }
  }
  ...
  static KeystoreException wrapKeystoreException(Exception e) {
    if (e != null) {
      ...
      if (msg.contains("java.lang.ArithmeticException: / by zero") || msg.contains("java.security.UnrecoverableKeyException: Get Key failed: / by zero")) { return new InvalidKeystorePasswordException("Blank passwords not supported (#38).  You must create your keystore with a non-empty password."); }
    }
    return new KeystoreException("Keystore exception: " + (e == null ? "no exception" : e.getMessage()), e);
  }
   ...
   *          by Apple (File, InputStream, byte[], KeyStore or String for a file
   *          path)
   * @throws javapns.communication.exceptions.InvalidKeystoreReferenceException
