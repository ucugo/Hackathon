 ...
 * 
 * Communication protocol differences between Notification and Feedback servers
 * are implemented in
 * {@link hackaton.notification.ConnectionToNotificationServer} and
 * {@link hackaton.feedback.ConnectionToFeedbackServer}.
 * 
...
public abstract class ConnectionToAppleServer {
   ...
   */
  public SSLSocket getSSLSocket() throws KeystoreException, hackaton.communication.exceptions.CommunicationException {
    SSLSocketFactory socketFactory = getSSLSocketFactory();
    logger.debug("Creating SSLSocket to " + getServerHost() + ":" + getServerPort());

    try {
      if (ProxyManager.isUsingProxy(server)) { return tunnelThroughProxy(socketFactory); }
      return (SSLSocket) socketFactory.createSocket(getServerHost(), getServerPort());
    } catch (Exception e) {
      throw new hackaton.communication.exceptions.CommunicationException("Communication exception: " + e, e);
    }
  }
  ...
  private SSLSocket tunnelThroughProxy(SSLSocketFactory socketFactory) throws UnknownHostException, IOException {
    ...
    try (Socket tunnel = new Socket(tunnelHost, tunnelPort);) {
  ...
  private void doTunnelHandshake(Socket tunnel, String host, int port) throws IOException {
    try (OutputStream out = tunnel.getOutputStream();) {

      String msg = "CONNECT " + host + ":" + port + " HTTP/1.0\n" + "User-Agent: BoardPad Server" + "\r\n\r\n";
      byte b[] = null;
      try { // We really do want ASCII7 -- the http protocol doesn't change with
            // locale.
        b = msg.getBytes("ASCII7");
      } catch (UnsupportedEncodingException ignored) { // If ASCII7 isn't there,
                                                       // something serious is
                                                       // wrong, but Paranoia Is
                                                       // Good (tm)
        b = msg.getBytes();
      }
      out.write(b);
      ...
      // We need to store the reply so we can create a detailed error message to
      // the user.
      ...
      try (InputStream in = tunnel.getInputStream();) {
      ...
      /*
       * Converting the byte array to a string is slightly wasteful in the case
       * where the connection was successful, but it's insignificant compared to
       ...
       * instead of 1.0
       */
      if (replyStr.toLowerCase().indexOf("200 connection established") == -1) { throw new IOException("Unable to tunnel through. Proxy returns \"" + replyStr + "\""); }
