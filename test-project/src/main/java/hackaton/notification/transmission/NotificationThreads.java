 ...
 * @see hackaton.notification.transmission.NotificationThread.MODE
...
public class NotificationThreads extends ThreadGroup implements PushQueue {
   ...
   */
  public NotificationThreads(hackaton.notification.AppleNotificationServer server, hackaton.notification.Payload payload, List<hackaton.devices.Device> devices, int numberOfThreads) {
    super("javapns notification threads (" + numberOfThreads + " threads)");
    for (List<hackaton.devices.Device> deviceGroup : makeGroups(devices, numberOfThreads))
      threads.add(new NotificationThread(this, new hackaton.notification.PushNotificationManager(), server, payload, deviceGroup));
  }
   ...
   * @param numberOfThreads the number of threads to create to share the work
   */
  public NotificationThreads(hackaton.notification.AppleNotificationServer server, List<hackaton.notification.PayloadPerDevice> messages, int numberOfThreads) {
    super("javapns notification threads (" + numberOfThreads + " threads)");
    for (List<hackaton.notification.PayloadPerDevice> deviceGroup : makeGroups(messages, numberOfThreads))
      threads.add(new NotificationThread(this, new hackaton.notification.PushNotificationManager(), server, deviceGroup));
  }
   ...
   * @throws Exception 
   */
  public NotificationThreads(Object keystore, String password, boolean production, hackaton.notification.Payload payload, List<hackaton.devices.Device> devices, int numberOfThreads) throws Exception {
    this(new hackaton.notification.AppleNotificationServerBasicImpl(keystore, password, production), payload, devices, numberOfThreads);
  }
   ...
   */
  public NotificationThreads(hackaton.notification.AppleNotificationServer server, hackaton.notification.Payload payload, List<hackaton.devices.Device> devices, List<NotificationThread> threads) {
    super("javapns notification threads (" + threads.size() + " threads)");
    this.threads = threads;
    List<List<hackaton.devices.Device>> groups = makeGroups(devices, threads.size());
   ...
   * @throws Exception 
   */
  public NotificationThreads(Object keystore, String password, boolean production, hackaton.notification.Payload payload, List<hackaton.devices.Device> devices, List<NotificationThread> threads) throws Exception {
    this(new hackaton.notification.AppleNotificationServerBasicImpl(keystore, password, production), payload, devices, threads);
  }
   ...
   * @param threads a list of pre-built threads
   */
  public NotificationThreads(hackaton.notification.AppleNotificationServer server, hackaton.notification.Payload payload, List<NotificationThread> threads) {
    super("javapns notification threads (" + threads.size() + " threads)");
    this.threads = threads;
  }
   ...
   * @throws Exception 
   */
  public NotificationThreads(Object keystore, String password, boolean production, hackaton.notification.Payload payload, List<NotificationThread> threads) throws Exception {
    this(new hackaton.notification.AppleNotificationServerBasicImpl(keystore, password, production), payload, threads);
  }
   ...
   */
  public NotificationThreads(hackaton.notification.AppleNotificationServer server, int numberOfThreads) {
    super("javapns notification thread pool (" + numberOfThreads + " threads)");
    for (int i = 0; i < numberOfThreads; i++) {
      threads.add(new NotificationThread(this, new hackaton.notification.PushNotificationManager(), server));
    }
  }

  @Override
  public PushQueue add(hackaton.notification.Payload payload, String token) throws hackaton.devices.exceptions.InvalidDeviceTokenFormatException {
    return add(new hackaton.notification.PayloadPerDevice(payload, token));
  }

  @Override
  public PushQueue add(hackaton.notification.Payload payload, hackaton.devices.Device device) {
    return add(new hackaton.notification.PayloadPerDevice(payload, device));
  }

  @Override
  public PushQueue add(hackaton.notification.PayloadPerDevice message) {
   ...
   * @return
   */
  private static <V> List<List<V>> makeGroups(List<V> objects, int threads) {
    List<List<V>> groups = new Vector<>(threads);
    ...
    for (int i = 0; i < threads; i++) {
      ...
      List<V> threadObjects = objects.subList(firstObject, lastObject);
   ...
   */
  @Override
  public hackaton.notification.PushedNotifications getPushedNotifications() {
    int capacity = 0;
    for (NotificationThread thread : threads)
      capacity += thread.getPushedNotifications().size();
    hackaton.notification.PushedNotifications all = new hackaton.notification.PushedNotifications(capacity);
   ...
   */
  public hackaton.notification.PushedNotifications getFailedNotifications() {
   ...
   */
  public hackaton.notification.PushedNotifications getSuccessfulNotifications() {
