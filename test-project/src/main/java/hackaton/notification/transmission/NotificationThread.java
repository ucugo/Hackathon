 ...
 */
public class NotificationThread implements Runnable, hackaton.notification.transmission.PushQueue {
  ...
  private hackaton.notification.PushedNotifications notifications = new hackaton.notification.PushedNotifications();
   ...
   * @param notificationManager the notification manager to use
   * @param server the server to communicate with
   * @param messages a list or an array of PayloadPerDevice: {@link java.util.List}<{@link hackaton.notification.PayloadPerDevice}>, {@link hackaton.notification.PayloadPerDevice PayloadPerDevice[]} or {@link hackaton.notification.PayloadPerDevice}
   */
  ...
  @Override
  public hackaton.notification.transmission.PushQueue add(Payload payload, String token) throws hackaton.devices.exceptions.InvalidDeviceTokenFormatException {
    return add(new PayloadPerDevice(payload, token));
  }

  @Override
  public hackaton.notification.transmission.PushQueue add(Payload payload, Device device) {
    return add(new PayloadPerDevice(payload, device));
  }

  @Override
  public hackaton.notification.transmission.PushQueue add(PayloadPerDevice message) {
    if (mode != MODE.QUEUE)
      return this;
    try {
  ...
  public List<Exception> getCriticalExceptions() {
    List<Exception> exceptions = new Vector<>(exception == null ? 0 : 1);
    if (exception != null)
