...
public abstract class Payload {
   ...
   * @throws hackaton.notification.exceptions.PayloadMaxSizeExceededException
   *           if the payload exceeds the maximum size allowed
   */
  private void validateMaximumPayloadSize(int currentPayloadSize) throws hackaton.notification.exceptions.PayloadMaxSizeExceededException {
    int maximumPayloadSize = getMaximumPayloadSize();
    if (currentPayloadSize > maximumPayloadSize) { throw new hackaton.notification.exceptions.PayloadMaxSizeExceededException(maximumPayloadSize, currentPayloadSize); }
  }
  ...
  protected void put(String propertyName, Object propertyValue, JSONObject object, boolean opt) throws JSONException {
    try {
      if (isPayloadSizeEstimatedWhenAdding()) {
        ...
        if (estimatedToExceed) throw new hackaton.notification.exceptions.PayloadMaxSizeProbablyExceededException(maximumPayloadSize, estimatedPayloadSize);
      }
    } catch (hackaton.notification.exceptions.PayloadMaxSizeProbablyExceededException e) {
      throw e;
    } catch (Exception e) {
      // swallow
    }
